...
const ResultsDisplay = () => {
  ...
  const matchedImageNames = filteredResults.map(r => r.imagename.trim());
  const matchedImages = imageDataArray.filter(image =>
    matchedImageNames.includes(image.name.replace('.jpg', '').trim())
  );
    ...
    return (
      <div className="text-[black] flex flex-col items-center content-center">
        <p className="text-sm italic text-gray-500 mb-4">
          Showing {matchedImages.length} image{matchedImages.length !== 1 ? 's' : ''}
        </p>
        <p className="text-gray-700 mb-4">{matchedImages.length} result(s) found.</p>
