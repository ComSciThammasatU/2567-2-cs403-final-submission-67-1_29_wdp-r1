...
//     const datetimePart = cleanString.split("D")[1]; // แยกหลัง "D"
...
const DisplayAllImagesFromLocalStorage = () => {
  ...
  useEffect(() => {
    const images = localStorage.getItem("imageDataArray");
    const dynamoData = localStorage.getItem("esp32sensordataData");

    if (images) {
      const parsed = JSON.parse(images);
      parsed.sort((a, b) => new Date(a.name) - new Date(b.name));
      setImageDataArray(parsed);
    }

    if (dynamoData) {
      setDynamoDBData(JSON.parse(dynamoData));
    }
  }, []);

  useEffect(() => {
    if (selectedImage && dynamoDBData.length > 0) {
      const imageName = selectedImage.name.replace(".jpg", "");
      const matched = dynamoDBData.filter((item) => item.imagename === imageName);
      setRelatedDynamoDBData(matched);
    } else {
      setRelatedDynamoDBData(null);
    }
  }, [selectedImage, dynamoDBData]);
  ...
  const handleFavorite = () => {
    const exists = favorites.find((f) => f.name === selectedImage.name);
    const updated = exists
      ? favorites.filter((f) => f.name !== selectedImage.name)
  ...
  const renderGroupedImages = () => {
    const grouped = groupImagesByDate();
    ...
    return (
      <div className="text-black flex flex-col items-center">
        ...
        {sortedDates.map((date) => (
          <div key={date}>
            <h3 className="text-lg font-semibold text-center my-2">{date}</h3>
            <div className="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-4">
              {grouped[date].map((image, index) => (
                <div
                  key={index}
                  className="bg-gray-100 p-4 rounded-lg cursor-pointer shadow md:w-[516px] md:h-[451px]"
  ...
  return (
    <>
      {renderGroupedImages()}
      {selectedImage && (
        <div className="fixed inset-0 w-full h-full bg-black bg-opacity-75 flex items-center justify-center px-4 py-6 z-50">
          <div className="bg-white p-4 rounded-lg max-w-lg mx-auto">
            <p className="text-center mb-2">
              Date: {formatDateTime(selectedImage.name).formattedDate}
              <br />
            ...
            <button
              ...
              className="block w-full mt-2 px-4 py-2 text-white rounded-md"
              style={{
                backgroundColor: favorites.some((f) => f.name === selectedImage.name)
                  ? "#d9534f"
                  : "#5cb85c",
              }}
            >
              {favorites.some((f) => f.name === selectedImage.name)
                ? "Remove from Favorites"
                : "Add to Favorites"}
            </button>
            ...
            {relatedDynamoDBData && relatedDynamoDBData.length > 0 && (
              <div className="mt-4">
                {relatedDynamoDBData.map((data, idx) => (
                  <div key={idx} className="flex flex-col items-center justify-center h-[250px]">
